...
function Field({
  ...
  return (
    <div className="relative pb-6">
      ...
      <label
        htmlFor={id}
        className="absolute left-0 top-3 text-[#1A1A1A]/40 text-xs tracking-widest uppercase font-['Manrope'] transition-all peer-placeholder-shown:top-3 peer-placeholder-shown:text-xs peer-focus:-top-3 peer-focus:text-[10px] peer-focus:text-[#C5A059] text-[10px]"
      >
        {label}
      </label>
      {error && (
        <p className="absolute bottom-1 text-red-400 text-[10px] font-['Manrope']">
          {error}
        </p>
      )}
    </div>
...
export default function ShippingForm({ register, errors }: Props) {
  return (
    <div>
      <h2 className="font-['Noto_Serif'] text-2xl text-[#1A1A1A] mb-8">
        Delivery Details
      </h2>
      <div className="grid grid-cols-2 gap-x-4">
        <Field
          id="firstName"
          label="First Name"
          register={register("shippingAddress.firstName", {
            required: "Required",
          })}
        ...
        <Field
          id="lastName"
          label="Last Name"
          register={register("shippingAddress.lastName", {
            required: "Required",
          })}
