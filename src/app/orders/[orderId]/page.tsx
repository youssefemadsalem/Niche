...
export default async function OrderDetailPage({
  ...
  await connectDB();
  const raw = await Order.findById(orderId).lean();
  if (!raw) notFound();
    ...
    <main className="min-h-screen bg-[#F9F8F6]">
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <div className="mb-8">
          ...
          <h1 className="font-['Noto_Serif'] text-3xl text-[#1A1A1A]">
            {order.orderNumber}
          </h1>
        </div>
        ...
        <div className="bg-white border border-[#E5D5B0] rounded-lg p-6 mb-6">
          <h2 className="font-['Noto_Serif'] text-lg text-[#1A1A1A] mb-5">
            Items
          </h2>
          <div className="space-y-4">
            {order.items.map((item: any, i: number) => (
              <div
                key={i}
                className="flex gap-4 items-center py-3 border-b border-[#E5D5B0] last:border-0"
              >
                <div className="relative w-14 h-16 bg-[#E5D5B0] rounded overflow-hidden shrink-0">
                  {item.image && (
                    <Image
                      src={item.image}
                      alt={item.name}
                ...
                <div className="flex-1">
                  <p className="text-xs text-[#C5A059] font-['Manrope'] uppercase tracking-wider">
                    {item.brand}
                  </p>
                  <p className="font-['Noto_Serif'] text-[#1A1A1A]">
                    {item.name}
                  </p>
                ...
                <p className="font-['Manrope'] text-[#1A1A1A]">
          ...
          <div className="mt-4 pt-4 border-t border-[#E5D5B0] space-y-2 text-sm font-['Manrope']">
            ...
            <div className="flex justify-between text-[#1A1A1A] font-medium pt-2 border-t border-[#E5D5B0]">
              ...
              <span className="font-['Noto_Serif'] text-lg">
        ...
        <div className="bg-white border border-[#E5D5B0] rounded-lg p-6">
          <h2 className="font-['Noto_Serif'] text-lg text-[#1A1A1A] mb-3">
            Delivery Address
          </h2>
