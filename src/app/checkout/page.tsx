...
export default function CheckoutPage() {
  ...
  const { items, subtotal, shipping, tax, promoDiscount, promoCode, finalTotal, clearCart } = useCart();

  const [step, setStep]       = useState<0 | 1 | 2>(0);
  const [loading, setLoading] = useState(false);
  const [error, setError]     = useState('');

  const { register, watch, handleSubmit, trigger, formState: { errors } } =
  ...
  const nextStep = async () => {
    const fields = step === 0
      ? ['shippingAddress.firstName','shippingAddress.lastName','shippingAddress.street','shippingAddress.city','shippingAddress.postalCode','shippingAddress.country'] as const
  ...
  return (
    <main className="min-h-screen bg-[#F9F8F6]">
      ...
      <header className="border-b border-[#E5D5B0] bg-white px-6 py-4 flex items-center justify-between">
        <Link href="/cart" className="flex items-center gap-2 text-[#1A1A1A]/50 hover:text-[#1A1A1A] transition-colors">
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            <path d="M10 2L4 8l6 6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
          </svg>
        </Link>
        <span className="font-['Noto_Serif'] text-lg tracking-[0.2em] text-[#1A1A1A]">NICHE</span>
        <div className="flex items-center gap-1 text-[#1A1A1A]/40">
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
            ...
            <path d="M4 5V4a3 3 0 016 0v1" stroke="currentColor" strokeWidth="1.2" />
          </svg>
          <span className="text-xs font-['Manrope'] tracking-widest uppercase">Secure Checkout</span>
        </div>
      </header>
      ...
      <div className="flex border-b border-[#E5D5B0] bg-white">
        {STEPS.map((s, i) => (
          <div key={s} className="flex-1 text-center py-3">
            <p className={`text-[10px] tracking-widest uppercase font-['Manrope'] ${i === step ? 'text-[#1A1A1A]' : 'text-[#1A1A1A]/30'}`}>
              Step {i + 1}
            </p>
            <p className={`text-sm font-['Noto_Serif'] ${i === step ? 'text-[#1A1A1A]' : 'text-[#1A1A1A]/30'}`}>{s}</p>
            {i === step && <div className="mt-1 h-0.5 bg-[#C5A059] mx-auto w-8" />}
          </div>
        ))}
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8 items-start">
        <div className="bg-white border border-[#E5D5B0] rounded-lg p-6 md:p-8">
          <form onSubmit={handleSubmit(onSubmit)} noValidate>
            ...
            {step === 1 && <PaymentMethodSelector register={register} watch={watch} />}
            {step === 2 && (
              <div>
                <h2 className="font-['Noto_Serif'] text-2xl text-[#1A1A1A] mb-4">Review Order</h2>
                <p className="text-sm text-[#1A1A1A]/60 font-['Manrope']">
                  Please review your items in the summary panel before completing your purchase.
                </p>
              </div>
            )}
            ...
            <div className="mt-8 flex gap-3">
              ...
              {step < 2 ? (
                ...
                <button type="submit" disabled={loading}
                  ...
                  {loading ? 'Processing...' : paymentMethod === 'card' ? 'Pay with Stripe →' : 'Place Order →'}
                </button>
              )}
            </div>
          </form>
